...
document.getElementById('form-conta').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        showNotification('Criando conta bancária com agência automática...', 'info');
        ...
        document.getElementById('cpf-validation').textContent = '';
        document.getElementById('cnpj-validation').textContent = '';
...
async function buscarConta() {
    ...
    try {
        ...
        if (!resultado.success) {
            throw new Error(resultado.message || 'Erro ao buscar conta');
        }
        ...
        resultadoDiv.innerHTML = `
            <div class="conta-info">
                <div class="info-card" style="background: linear-gradient(135deg, #e3f2fd, #bbdefb); border-left: 4px solid #2196f3;">
                    ...
                    <div class="info-item">
                        <strong>Agência:</strong> ${conta.agencia || 'Não informada'}
                    </div>
                    <div class="info-item">
                ...
                <div class="info-card" style="background: linear-gradient(135deg, #e8f5e8, #c8e6c9); border-left: 4px solid #4caf50;">
                    <h3>👤 Informações do Cliente</h3>
                    <div class="info-item">
                        <strong>Cliente:</strong> ${conta.nomeCliente}
                    </div>
                </div>
                <div class="info-card" style="background: linear-gradient(135deg, #f3e5f5, #e1bee7); border-left: 4px solid #9c27b0;">
        ...
        document.getElementById('resultado-busca').innerHTML = `
            <div class="error-message" style="text-align: center; color: #e74c3c; padding: 20px; background: #fde8e8; border-radius: 8px; border-left: 4px solid #e74c3c;">
                <h3>❌ Conta não encontrada</h3>
                <p>${error.message}</p>
                <p><small>Verifique se o número da conta está correto.</small></p>
            </div>
        `;
...
async function listarContas() {
    try {
        ...
        if (!resultado.success) {
            throw new Error(resultado.message || 'Erro ao carregar contas');
        }
        ...
        if (contas.length === 0) {
            listaDiv.innerHTML = '<div class="loading" style="text-align: center; padding: 40px; color: #7f8c8d;">Nenhuma conta cadastrada</div>';
        ...
        showNotification(`${contas.length} contas carregadas com sucesso!`, 'success');
        
    } catch (error) {
        document.getElementById('lista-contas').innerHTML = `
            <div style="text-align: center; color: #e74c3c; padding: 20px; background: #fde8e8; border-radius: 8px;">
...
async function listarClientes() {
    try {
        ...
        if (clientes.length === 0) {
            listaDiv.innerHTML = '<div class="loading" style="text-align: center; padding: 40px; color: #7f8c8d;">Nenhum cliente cadastrado</div>';
        ...
        listaDiv.innerHTML = `
            <div style="margin-bottom: 20px;">
                ...
                <div style="background: #f8f9fa; padding: 10px 15px; border-radius: 8px; display: inline-block;">
...
async function listarBancos() {
    try {
        ...
        if (!resultado.success) {
            throw new Error(resultado.message || 'Erro ao carregar bancos');
        }
        ...
        if (bancos.length === 0) {
            listaDiv.innerHTML = '<div class="loading" style="text-align: center; padding: 40px; color: #7f8c8d;">Nenhum banco cadastrado</div>';
...
async function testarConexao() {
    try {
        // Tentar buscar a lista de bancos como teste de conexão
        const response = await fetch(`${API_BASE_URL}/bancos`);
        ...
        if (response.ok) {
            ...
            throw new Error('Backend não responde corretamente');
        }
    } catch (error) {
        const statusIndicator = document.getElementById('status-indicator');
        const statusText = document.getElementById('status-text');
...
function aplicarMascaras() {
    ...
    // Permitir buscar conta com Enter
    const numeroBuscaInput = document.getElementById('numeroBusca');
    if (numeroBuscaInput) {
        numeroBuscaInput.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                buscarConta();
            }
        });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const tabButton = document.querySelector(`[onclick="openTab('${activeTab}')"]`);
    if (tabButton) {
        tabButton.click();
    } else {
        // Fallback para a primeira tab
...
// Função para mostrar loading em qualquer botão
function mostrarLoading(botao, texto = 'Carregando...') {
    const originalHTML = botao.innerHTML;
    botao.innerHTML = `<span class="loading-spinner"></span> ${texto}`;
    botao.disabled = true;
    return originalHTML;
}

// Função para esconder loading
function esconderLoading(botao, originalHTML) {
